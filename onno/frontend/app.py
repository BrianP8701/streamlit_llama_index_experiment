...
if 'all_terms' not in st.session_state:
    st.session_state['all_terms'] = DEFAULT_TERM_STR
        ...
        if data_type == "pdf":
            ...
            if uploaded_file:
                ...
                with st.spinner("Extracting..."):
                    terms_docs.update(extract_terms([Document(file=uploaded_file)], term_extract_str, llm_name, model_temperature, api_key))
            ...
            if url:
                ...
                terms_docs = {}
                with st.spinner("Extracting..."):
                    terms_docs.update(extract_terms([Document(url=url)], term_extract_str, llm_name, model_temperature, api_key))
                st.session_state['terms'].update(terms_docs)
            ...
            if github_repo:
                ...
                terms_docs = {}
                with st.spinner("Extracting..."):
                    terms_docs.update(extract_terms([Document(github_repo=github_repo)], term_extract_str, llm_name, model_temperature, api_key))
                st.session_state['terms'].update(terms_docs)
